...
#include <string.h>
#include <termios.h>
#include <SGPM01_imu/include/SpgM01.h>
#include <unistd.h>


int SpgM01::SetOpt(int fd,int nSpeed, int nBits, char nEvent, int nStop)
{
...
int SpgM01::ParseData(unsigned char & data,int n){
    int nByte ;
    short sum_1;
    int cnt=0;
    int err=0;
    ...
    {	
        ...
        {
            buf[i+cnt] = buffer[i];
            //printf("%02x,",buffer[i]); //可以打开打印//ff,ff,00,00,0b,00,00,00,00,00,01,00,15,00,f0,03,10,00,3b,21,22,16,36,ce,56,14,26,03,0d,0a
